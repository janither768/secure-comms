...
let db = {};
let activeUsers = {}; 
let roomConstraints = {}; // { "Room": "TargetAlias" }
const HEARTBEAT_MS = 45000;

// --- STYLING: Clean, Minimal, Proven ---
...
<body style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100vh;">
  ...
  <button class="btn-tactical" onclick="window.location.href='/boot'">[ ENGAGE UPLINK ]</button>
</body></html>`;
...
const renderChat = (user, room) => {
  ...
  <body style="padding-bottom:150px;">
    <div style="background:#11151c; border-bottom:1px solid #1f2937; padding:15px; display:flex; justify-content:space-between; align-items:center;">
       ...
       <div style="font-size:0.7em; color:${isSecure?'#39ff14':'#5c748c'}; font-weight:bold;">${isSecure?'LINK SECURE':'AWAITING'} ●</div>
    </div>
    ...
    <div style="position:fixed; bottom:0; width:100%; background:#11151c; border-top:1px solid #2d3748; padding:10px; text-align:center; box-sizing:border-box;">
       ...
       <a href="data:text/plain;base64,${encodedExport}" download="chat.txt" style="color:#5c748c; font-size:0.8em;">[ DOWNLOAD ]</a>
       <span style="color:#2d3748; margin:0 5px;">|</span>
       <a href="/chat?user=${user}&room=${room}" style="color:#5c748c; font-size:0.8em;">[ PING ]</a>
       <span style="color:#2d3748; margin:0 5px;">|</span>
       <a href="/purge?room=${room}" style="color:#ff4c4c; font-size:0.8em; font-weight:bold;">[ PURGE ]</a>
    </div>
  </body></html>`;
};
...
app.get('/chat', (req, res) => {
  ...
  if (roomConstraints[room] && user !== roomConstraints[room] && (!db[room] || !db[room].some(m=>m.sender===roomConstraints[room]))) {
